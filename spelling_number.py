...
def spelling_number(string, spl_token='|', royal_society=False):
    ...
    while (start_index >= 0):
        result = spell_digit(
            digit[start_index:end_index], is_million=is_million) + result
        end_index = start_index
        start_index -= 6
        is_million = True

    if (start_index > -6):
        result = spell_digit(digit[0:end_index],
                             is_million=is_million) + result
    ...
    if (spl_token != ""):
        ...
        for word in string.split(spl_token):
            if (word.replace(',', '').replace('.', '').isdigit()):
                result += spelling_number(word,
                                          spl_token=spl_token) + spl_token
            else:
        ...
        for i in range(len(string)):
            if (string[i].isdigit()):
                ...
                if ((i+1 < len(string)) and (string[i] in ['.', ',']) and (string[i+1].isdigit())):
                    continue
                result = result + string[end_ind:start_ind] + \
                    spelling_number(string[start_ind:i], spl_token=spl_token)
                end_ind = i
                start_ind = -1

        if (start_ind != -1):
            result = result + string[end_ind:start_ind] + \
                spelling_number(string[start_ind:], spl_token=spl_token)
        else:
